      ...
      {projectsArray.map((project) => (
        <li key={project.id}>
          ...
          <div className="flex gap-4 mt-6 flex-col lg:flex-row justify-between items-start lg:items-end">
            <div className="flex flex-col items-start lg:max-w-[80%]">
              <div className="font-semibold text-sm px-2 mb-2 rounded-full rounded-bl-none bg-slate-200">
                {project.tag}
              </div>
              <h3 className="text-xl lg:text-3xl font-bold">{project.title}</h3>
            ...
            <a
              ...
              href={project.href}
              className="group text-slate-900 whitespace-pre font-medium cursor-pointer flex items-center gap-2">
              <span>View live</span>
              <FontAwesomeIcon
